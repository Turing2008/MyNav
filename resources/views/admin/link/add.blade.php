        ...
        <div class="layui-row">
            <form class="layui-form" action="" lay-filter="example" onsubmit="return false;">
                    ...
                    <div class="layui-input-block ">
                        <select id="survey_id" name="Link[category_id]" lay-filter="survey_id" class=" width-120">
                            ...
                            @foreach($categories as $category)
                                <option data-name="" value="{{$category->id}}"
                                        @if($category->id==$link->category_id)selected="selected"@endif>{!! $category->name !!}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="radio" name="Link[flag]" value="1" title="是">
                        <input type="radio" name="Link[flag]" value="0" title="否" checked>
                    </div>
                </div>
                <div class="layui-form-item layui-form-text">
                    <label class="layui-form-label">描述 <span class="color-red"></span></label>
                    <div class="layui-input-block">
                            <textarea placeholder="" class="layui-textarea" name="Link[description]"
                                      maxlength="500">{{$link->description ?? ''}}</textarea>
                    </div>
                </div>
